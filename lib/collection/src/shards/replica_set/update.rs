...
impl ShardReplicaSet {
    ...
    async fn update(
        ...
        let clock_sync = vector_clock_guard.get_clock_sync(this_peer_id);
        ...
        let all_res: Vec<Result<_, _>> = {
            ...
            if let Some(local) = local.deref() {
                if self.peer_is_active_or_pending(&this_peer_id) {
                    ...
                    let clock_sync_clone = clock_sync.clone();
                    let local_update = async move {
                        local
                            .get()
                            .update(operation, local_wait, Some(clock_sync_clone))
                            .await
            ...
            for remote in active_remote_shards {
                ...
                let remote_update = async move {
                    remote
                        .update(operation, wait, Some(clock_sync_clone))
                        .await
    ...
    async fn forward_update(
        ...
        match remote_leader {
            Some(remote_leader) => {
                // Once we forward the update to the leader replica, we assume that the clock tick
                // will be generated by the leader replica, so we don't need to generate it here.
                let clock_sync = None;
                remote_leader
                    .forward_update(operation, wait, ordering, clock_sync)
                    .await
            }
            None => Err(CollectionError::service_error(format!(
